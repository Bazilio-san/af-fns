...
module.exports = {
  ...
  xxHash (data, base = '32', seed = 0xCAFEBABE) {
    ...
    if (data === undefined) {
      ...
      const arr = data.map((value) => ([value, this.xxHash32(String(value) + (typeof value), seed)]));
      arr.sort(([, a], [, b]) => {
        if (a < b) return -1;
        if (a > b) return 1;
        return 0;
    ...
    switch (String(base)) {
      case '64':
        hash = XXH.h64(stringToHash, seed)
          .toString(16);
        break;
      default:
        hash = XXH.h32(stringToHash, seed)
          .toString(16);
    }
    return hash;
  },
  ...
  xxHash32 (data, seed = 0xCAFEBABE) {
    return this.xxHash(data, 32, seed)
      .toString(16);
  },

  xxHash64 (data, seed = 0xCAFEBABE) {
    return this.xxHash(data, 64, seed)
      .toString(16);
  },
   ...
   * @returns {String}
   */
  fd (dt, format = 'YYYY-MM-DD HH:mm', timeZone = undefined) {
    if (!dt) {
      return null;
    }
    let mom;
  ...
  parsePairs (str, delimBetweenPairs = '&', delimBetweenNameAndValue = '=') {
    const pairsArray = str.split(delimBetweenPairs)
      .filter((p) => p.trim())
      .map((p) => (p.split(delimBetweenNameAndValue)
        .map((v) => (v || '').trim())))
      .filter((p) => (p[1] || '').trim());
    const result = {};
    pairsArray.forEach(([p, v]) => {
      result[p] = v;
    });
    return result;
  },
